...
void make_idle_task(){
    idle_process = malloc(sizeof(struct process));
    idle_process->id = 0;
    idle_process->state = RUNNING;
    idle_process->pml4 = TASK0_PML4;
    idle_process->kstack = (uint64_t)&task0_stack;
    idle_process->rsp0 = (uint64_t)&task0_stack;
    idle_process->rip = (uint64_t)&idle_task_entry;

    // fake_task_stack(idle_process->kstack);
    // idle_process->rsp0 = idle_process->kstack - 8*5;
    // idle_process->rip = (uint64_t)&ret_from_kernel;
}
